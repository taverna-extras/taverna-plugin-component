package net.sf.taverna.t2.component.registry.myexperiment.client.utils;
...
public class Base64 {
	...
	private static Logger logger = getLogger(Base64.class);
	...
	private static int decode4to3(byte[] source, int srcOffset,
		...
		// Example: DkLE
			...
			try {
				...
				logger.error("" + source[srcOffset] + ": "
						+ (DECODABET[source[srcOffset]]), e);
				logger.error("" + source[srcOffset + 1] + ": "
						+ (DECODABET[source[srcOffset + 1]]), e);
				logger.error("" + source[srcOffset + 2] + ": "
						+ (DECODABET[source[srcOffset + 2]]), e);
				logger.error("" + source[srcOffset + 3] + ": "
	...
	public static byte[] decode(byte[] source, int off, int len, int options) {
		...
		for (i = off; i < off + len; i++) {
												...
												// better
			{
				...
				logger.error("Bad Base64 input character at " + i + ": "
						+ source[i] + "(decimal)");
				return null;
			} // end else:
	...
	public static byte[] decodeFromFile(String filename) {
		...
		try {
			...
			if (file.length() > Integer.MAX_VALUE) {
				logger.error("File is too big for this convenience method ("
						+ file.length() + " bytes).");
				return null;
